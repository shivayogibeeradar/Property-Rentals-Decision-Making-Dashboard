...
def roi_with_equity(df,n):
    return((((((1+df["rate"]))**n)*df["annual_rent"])*n+(((1+df["rate"]))**n)*df["sell_price"]-df["sell_price"])/df["sell_price"])

def roi_without_equity(df,n):
    return(((((1+df["rate"]))**n)*df["annual_rent"])*n/df["sell_price"])